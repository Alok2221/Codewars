...
public class DerivativesOfTypeXPowerNTest {

    @ParameterizedTest(name = "Derivatives.differentiate(\"{0}\")")
    @ValueSource(strings = {"0", "1", "-1", "42", "-13"})
    void testConstant(String input) {
        assertEquals("0", DerivativesOfTypeXPowerN.differentiate(input), String.format("Incorrect answer for \"%s\"", input));
    }

    @ParameterizedTest(name = "Derivatives.differentiate(\"{0}\")")
    @CsvSource({"12x^3,36x^2", "-12x^3,-36x^2", "12x^-3,-36x^-4", "-12x^-3,36x^-4"})
    void testRegularCases(String input, String expected) {
        assertEquals(expected, DerivativesOfTypeXPowerN.differentiate(input), String.format("Incorrect answer for \"%s\"", input));
    }
